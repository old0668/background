...
class RoundedButton(tk.Canvas):
    def __init__(self, parent, text, command=None, width=105, height=36, radius=18, color="#2C2C2E", hover_color="#3A3A3C", text_color="white", font=("Inter", 11, "bold")):
...
class XiangTeacherProApp:
    def __init__(self, root):
        self.root = root
        self.root.title("祥老師去背 - Pro AI Production")
        ...
        self.right_click = "<Button-2>" if self.is_mac else "<Button-3>"
        self.middle_click = "<Button-3>" if self.is_mac else "<Button-2>"
    ...
    def setup_ui(self):
        ...
        title_bar = tk.Frame(self.container, bg=self.color_bg); title_bar.pack(fill=tk.X, padx=30, pady=10)
        tk.Label(title_bar, text="祥老師去背", bg=self.color_bg, fg="white", font=("PingFang TC", 28, "bold")).pack(side=tk.LEFT)
        ...
        style.configure("Pro.TRadiobutton", background=self.color_toolbar, foreground="white", font=("PingFang TC", 11))
        ...
        tk.Label(bg, text="筆刷:", bg=self.color_toolbar, fg="#8E8E93", font=("PingFang TC", 11)).pack(side=tk.LEFT, padx=5)
    ...
    def create_context_menu(self):
        self.context_menu = tk.Menu(self.root, tearoff=0, bg="#2C2C2E", fg="white", activebackground="#0A84FF", font=("PingFang TC", 11))
        ...
        self.context_menu.add_separator(); self.context_menu.add_command(label="✋ 移動視圖", command=lambda: self.quick_set_tool("pan"))
    ...
    def bind_events(self):
        ...
        self.root.bind("<Control-z>", lambda e: self.undo()); self.root.bind("<Command-z>", lambda e: self.undo())

    def load_image(self):
        path = filedialog.askopenfilename(filetypes=[("Images", "*.jpg *.jpeg *.png *.webp")])
        if path:
            self.original_img = Image.open(path).convert("RGB"); self.mask = Image.new("L", self.original_img.size, 255)
            self.history = []; self.reset_view(); self.btn_ai.set_state("normal")

    def update_brush_size(self, val): self.brush_size = int(float(val)); self.draw_brush_cursor_at_last_pos()
    def update_brush_cursor(self, event): self.last_mx, self.last_my = event.x, event.y; self.draw_brush_cursor_at_last_pos()
    ...
    def get_img_coords(self, sx, sy):
        w, h = self.original_img.size; ts = self.get_total_scale(); cw, ch = self.canvas_right.winfo_width() // 2, self.canvas_right.winfo_height() // 2
    ...
    def handle_click(self, event):
        ...
        elif self.tool_mode == "crop": self.crop_x1, self.crop_y1 = event.x, event.y; self.canvas_right.delete("crop_rect")
        else:
    ...
    def move_view(self, dx, dy): self.canvas_right.xview_scroll(dx, "units"); self.canvas_right.yview_scroll(dy, "units"); self.canvas_left.xview_scroll(dx, "units"); self.canvas_left.yview_scroll(dy, "units"); self.draw_brush_cursor_at_last_pos()
    ...
    def show_save_dialog(self):
        ...
        def up():
            ...
            else: ol.config(text="品質 (10-100):"); os.config(from_=10, to=100); os.set(85)
        tk.Radiobutton(sw, text="PNG", variable=fmt, value="PNG", bg="#2C2C2E", fg="white", selectcolor="#0A84FF", command=up).pack(); tk.Radiobutton(sw, text="JPG", variable=fmt, value="JPEG", bg="#2C2C2E", fg="white", selectcolor="#0A84FF", command=up).pack()
        ...
        def save():
            ...
            if p:
                ...
                sw.destroy()
                self.show_minimal_info("儲存成功", f"儲存完成！\n路徑：{p}")
    ...
    def show_minimal_info(self, title, msg):
        ...
        tk.Label(win, text=msg, bg="#1C1C1E", fg="white", font=("PingFang TC", 11), wraplength=400, justify=tk.LEFT, pady=30).pack()
...
if __name__ == "__main__":
    root = tk.Tk(); app = XiangTeacherProApp(root); root.mainloop()
